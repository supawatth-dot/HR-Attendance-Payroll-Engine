...
export function TopBar() {
  ...
  const getPageTitle = () => {
    ...
    if (pathname.includes('/attendance')) return 'Attendance Records & Results';
    if (pathname.includes('/import')) return 'Bulk Data Import (Excel/CSV/SAP)';
    if (pathname.includes('/rules')) return 'Dynamic Rule Engine & Policy Management';
    if (pathname.includes('/payroll')) return 'Payroll Calculation & Multi-Format Export';
    if (pathname.includes('/audit')) return 'Enterprise Immutable Audit Trail';
      ...
      <div>
        <h1 className="text-lg font-semibold text-neutral-100 flex items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-400 border border-emerald-500/20">
            <ShieldCheck size={12} /> Rule-Driven Engine
          </span>
        </h1>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-3 pl-3 border-l border-neutral-800">
          ...
          <div className="hidden md:block">
            <div className="text-xs font-semibold text-neutral-200">System Administrator</div>
            <div className="text-[10px] text-indigo-400 font-medium">Enterprise Policy 2026</div>
          </div>
        </div>
      </div>
    </header>
