...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Invalid username or password. Try username: System');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0a0a0f] px-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl glass-card p-8 border border-neutral-800/80 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-white">HR Engine Enterprise</h2>
          <p className="mt-1 text-xs text-neutral-400 font-medium flex items-center justify-center gap-1.5">
            <ShieldCheck size={14} className="text-emerald-400" />
            100% Database-Driven Dynamic Rule Policies
          </p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Enter 'System' or Employee ID"
                required
        ...
        <div className="mt-6 border-t border-neutral-800/80 pt-4 text-center">
          <span className="text-[11px] text-neutral-500 font-medium">
            Production environment default setup: login with username <strong className="text-neutral-300">System</strong>
          </span>
        </div>
      </div>
    </div>
